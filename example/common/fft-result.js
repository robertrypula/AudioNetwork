...
FFTResult.prototype.getLoudestBinIndex = function (frequencyStart, frequencyEnd) {
    return this.$$getLoudestBinIndexInRange(frequencyStart, frequencyEnd);
};

FFTResult.prototype.getLoudestFrequency = function (frequencyStart, frequencyEnd) {
    var loudestBinIndex = this.$$getLoudestBinIndexInRange(frequencyStart, frequencyEnd);

    return FFTResult.getFrequency(
        loudestBinIndex, this.$$sampleRate, this.$$fftSize
    );
};
...
FFTResult.prototype.getFrequency = function (frequencyBinIndex) {
    return FFTResult.getFrequency(frequencyBinIndex, this.$$sampleRate, this.$$fftSize);
};

FFTResult.prototype.getFrequencyOfClosestBin = function (frequency) {
    return FFTResult.getFrequencyOfClosestBin(frequency, this.$$sampleRate, this.$$fftSize);
};

FFTResult.prototype.getBinIndex = function (frequency) {
    return FFTResult.getBinIndex(frequency, this.$$sampleRate, this.$$fftSize);
};

FFTResult.prototype.getResolution = function () {
    return FFTResult.getFrequency(
        FFTResult.$$_FREQUENCY_BIN_INDEX_FIRST, this.$$sampleRate, this.$$fftSize
    );
};
...
FFTResult.prototype.getLastFrequency = function () {
    return this.getFrequency(this.getLastBinIndex());
};

FFTResult.prototype.getNyquistFrequency = function () {
    return 0.5 * this.$$sampleRate;
};
...
FFTResult.prototype.equal = function (fftResult) {
    var i, absDiff, isFrequencyEqual, isFFTSizeEqual, isAllDecibelEqual;

    isFrequencyEqual = this.getNyquistFrequency() === fftResult.getNyquistFrequency();
    isFFTSizeEqual = this.getFFTSize() === fftResult.getFFTSize();
    ...
    for (i = 0; i < this.$$fftData.length; i++) {
        absDiff = Math.abs(this.$$fftData[i] - fftResult.getDecibel(i));
...
FFTResult.prototype.$$getLoudestBinIndexInRange = function (frequencyStart, frequencyEnd) {
    var frequencyBinIndexStart, frequencyBinIndexEnd, loudestBinIndex;

    frequencyStart = FFTResult.$$getValueOrDefault(frequencyStart, FFTResult.$$_FREQUENCY_BIN_INDEX_ZERO);
    frequencyEnd = FFTResult.$$getValueOrDefault(frequencyEnd, this.getLastFrequency());
    frequencyBinIndexStart = this.getBinIndex(frequencyStart);
    frequencyBinIndexEnd = this.getBinIndex(frequencyEnd);

    loudestBinIndex = FFTResult.$$findMaxIndexInRange(this.$$fftData, frequencyBinIndexStart, frequencyBinIndexEnd);
